...
export default async function handler(req, res) {
  ...
  try {
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        // onboarding@resend.dev is Resend's shared sender, so no domain
        // verification is needed. The catch: an unverified Resend account can
        // only deliver to the address it was registered with, so this
        // recipient and the Resend account owner must be the same address
        // until a custom sending domain is verified.
